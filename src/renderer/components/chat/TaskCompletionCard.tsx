...
const REPORT_REGEX = /```laf-agent-report\s*\n([\s\S]*?)\n```/
const JSON_FENCE_REGEX = /```json\s*\n([\s\S]*?)\n```/
const BARE_JSON_REGEX = /\{[\s\S]*"status"\s*:\s*"(?:done|partial|blocked)"[\s\S]*"summary"\s*:\s*"[^"]+?"[\s\S]*\}/
const AGENT_SUMMARY_REGEX = /<agent_summary>[\s\S]*?<\/agent_summary>/g

const VALID_STATUSES = new Set(['done', 'partial', 'blocked'])

/** Validate that a parsed object is a LafAgentReport. */
const isValidReport = (obj: unknown): obj is LafAgentReport => {
  if (typeof obj !== 'object' || obj === null) return false
  const r = obj as Record<string, unknown>
  return typeof r.status === 'string' && VALID_STATUSES.has(r.status) && typeof r.summary === 'string' && r.summary.length > 0
}

/** Extract a completion report from message text. Checks laf-agent-report fences, json fences, and bare JSON. */
export const parseReport = (text: string): LafAgentReport | null => {
  // 0. <agent_summary> tag
  const summaryMatch = /<agent_summary>\s*([\s\S]*?)\s*<\/agent_summary>/.exec(text)
  if (summaryMatch) {
    try {
  ...
  // 1. laf-agent-report fence (highest priority)
...
export const shouldRenderReportCard = (report: LafAgentReport): boolean =>
...
export const stripReport = (text: string): string => {
  // Always strip <agent_summary> tags
  let cleaned = text.replace(AGENT_SUMMARY_REGEX, '').trimEnd()
...
export const TaskCompletionCard = memo(function TaskCompletionCard({ report }: { report: LafAgentReport }) {
