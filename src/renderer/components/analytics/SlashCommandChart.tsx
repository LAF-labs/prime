      ...
      <ResponsiveContainer width="100%" height={Math.max(100, sorted.length * 28 + 40)}>
        <BarChart data={sorted} layout="vertical" margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
          ...
          <Legend
            ...
            iconSize={8}
          />
          <Bar dataKey="command" name={t('Command')} stackId="mode" fill="var(--chart-3)" radius={[0, 0, 0, 0]} />
          <Bar dataKey="plan" name={t('Plan')} stackId="mode" fill="var(--chart-5)" radius={[0, 3, 3, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
  )
